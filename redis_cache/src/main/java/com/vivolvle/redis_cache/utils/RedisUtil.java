...
public class RedisUtil {
     ...
     * @param mapKey
     */
    public void incrementMap(String key, String mapKey) {
        redisTemplate.execute(new SessionCallback<Object>() {
            @Nullable
            @Override
            public String execute(RedisOperations redisOperations) throws DataAccessException {
                String value = (String) getMapKey(key, mapKey);
                int now = Integer.parseInt(value);
                now++;
                setMap(key, mapKey, String.valueOf(now));
                return "OK";
            }
        });
    }
    ...
    public void descHash(String key) {
        redisTemplate.execute(new SessionCallback<Object>() {
            @Nullable
            @Override
            public Object execute(RedisOperations redisOperations) throws DataAccessException {
                Integer result = Integer.parseInt(getString(key));
